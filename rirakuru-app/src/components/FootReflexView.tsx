...
import { footReflex, footSections } from "@/data/footReflex";
...
export function FootReflexView() {
  ...
  return (
    <div className="flex flex-col gap-5">
      {footSections.map((sec) => {
        const steps = footReflex.filter((s) => s.section === sec);
...
function buildOrderQA(): OrderQA[] {
  ...
  qa.push({
    q: "足つぼの一番最初は？",
    a: `手順1：${footReflex[0].name}（${footReflex[0].detail}）`,
  });
  // 各手順の「次」
  for (let i = 0; i < footReflex.length - 1; i++) {
    const cur = footReflex[i];
    const next = footReflex[i + 1];
