...
import { APPROACHES, PHASES } from '../src/data/approach.js';
import { CONSENT_POINTS } from '../src/data/consent.js';
...
test('カテゴリは定義済みで、飛び先の画面と anchor を持つ', () => {
  const views = new Set(['approach', 'tactics', 'consent', 'replies', 'habits', 'myths', 'sources']);
  for (const e of TOC_ENTRIES) {
...
test('目次はすべてのデータを漏れなく載せる', () => {
  const count = (id) => TOC_ENTRIES.filter((e) => e.category === id).length;
  const aliases = TACTICS.reduce((n, t) => n + (t.aka || []).length, 0);
  assert.equal(count('approach'), APPROACHES.length);
  assert.equal(count('phase'), PHASES.length);
  assert.equal(count('consent'), CONSENT_POINTS.length);
  ...
  assert.equal(
    TOC_ENTRIES.length,
    APPROACHES.length + PHASES.length + CONSENT_POINTS.length + TACTICS.length + aliases +
      CATEGORIES.length + REPLIES.length + HABITS.length + STATES.length + MYTHS.length + SOURCES.length,
  );
});
...
test('世に出回っている呼び名を目次から引ける', () => {
  ...
  for (const w of ['ラブボミング', '間欠強化', 'ガスライティング', 'ネグ', '希少性の原理', '返報性']) {
...
test('検索とカテゴリ絞り込みが効く', () => {
  assert.ok(filterToc(TOC_ENTRIES, { category: 'consent' }).every((e) => e.category === 'consent'));
  assert.ok(filterToc(TOC_ENTRIES, { query: '終電' }).length > 0);
