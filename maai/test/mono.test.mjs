...
import { PHASES, EVIDENCE } from '../src/data/approach.js';
...
test('後読み（lookbehind）を使わない（古い Safari が読み込みごと落ちる）', () => {
  for (const f of srcFiles()) {
    assert.ok(!read(f).includes('(?<'), `${f}: 後読みが使われています`);
  }
});
