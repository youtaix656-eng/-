...
test('知識の要約に、箇条書きの途中や表の行を拾わない', () => {
  // 一覧に出る1行なので、文になっていない行を拾うと意味が通らなくなる。
  // 実際に「1. 一次情報（施術者としての実感）が入っていること」が
  // 知識カードの要約になっていた。
  const body = [
    '## 調べたこと',
    '',
    '**買い手が求めていること**',
    '1. 一次情報が入っていること',
    '2. ガイドラインに触れない表現であること',
    '',
    '有資格者であることを明記した出品は単価が高くなる傾向がある。',
  ].join('\n');
  const { summary } = distill(body, 'テスト');
  assert.equal(summary, '有資格者であることを明記した出品は単価が高くなる傾向がある。');

  const table = ['## 整理', '', '| 項目 | 判断 |', '|---|---|', '| 単価 | 4円 |', '', '実績づくりを優先する。'].join('\n');
  assert.equal(distill(table, 'テスト').summary, '実績づくりを優先する。');

  // 全角の番号・記号でも同じ
  const zen = ['１． 最初の項目です', '', 'これが本文の1行目にあたる説明の文です。'].join('\n');
  assert.equal(distill(zen, 'テスト').summary, 'これが本文の1行目にあたる説明の文です。');
});
