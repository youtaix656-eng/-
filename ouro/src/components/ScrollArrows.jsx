// 画面の右端に置く、スクロール用の矢印。
//
// 長い画面（目次・社員・仕事の詳細）で、指を何度もはらわずに動かせるようにする。
//
// 決まりごと：
//  ・**動かない画面には出さない。** 中身が1画面に収まっている時は、押しても
//    何も起きないボタンになるので丸ごと隠す（行き止まりを作らない）。
//  ・端まで来た側の矢印は**消さずに薄くする**（位置が動くと押し損ねるため）。
//  ・`backdrop-filter` は付けない（新項目19：合成レイヤーは下部ナビ・会社バー・
//    上部バーだけ。増やすほど合成そのものが重くなる）。
//  ・重なりは会社バー（39）より下に置く。モーダルが開いたら隠れてよい。
