...
### 6-23. オフィスの様子（`lib/office.js` / `components/Office.jsx`）

**録画ではなく、いまの状態から毎回描く。**「働いている映像」を持つと、
実際には誰も動いていない時でも動いて見えてしまう。

```
tasks → presence.buildPresence(employees, tasks)   … その人の手順が走っているか
      → office.officeLayout(rows, ROLE_GROUPS, groupOf)
          islands[]  … チームごとの島（1列 PER_ROW=3 席）
          seats[]    … x/y・motion・moving・phase(id から決まる)
      → Office.jsx が線だけの SVG で描き、動きは CSS のアニメーション
```

| 状態 | 色 | 見え方 | 動くか |
|---|---|---|---|
| 実行中 | 緑 `#4ade80` | 腕がタイピング・首が動く・画面と印が明滅・線が濃い・担当の仕事名 | **○（本当にAIが動いている）** |
| 順番待ち | 水色 `#7dd3fc` | ごくわずかに揺れる・印「…」 | × |
| あなた待ち | 黄 `#fbbf24` | こちらを見上げたまま止まる・印「⚖」 | × |
| 保留 | 紫 `#c4b5fd` | 薄くして止める・印「‖」 | × |
| 止まっている | 赤 `#f87171` | 薄くして止める・印「⚠」 | × |
| 手あき | 灰 `#8b8b8b` | ゆっくり呼吸するだけ | ×（作業ではないと分かる速さ） |

色は `SEAT_LOOKS[].color`（`seatColor()`）が単一の正で、画面に色を直書きしない。
**色だけに意味を持たせない**——記号・名前・人数・動きを必ず一緒に出す。

| 決まり | なぜ |
|---|---|
| 画像・動画ファイルを持たない | 肖像と同じ線。起動の速さと直結（`docs/PERFORMANCE.md`） |
| 乱数を使わない（`phaseOf`） | 描き直すたびに全員が飛び跳ね、動きが意味を失う |
| 拍は席ごとにずらす | 全員が同じ拍だと機械に見える |
| `office.js` は時計を読まない | 経過秒は呼ぶ側が渡す（テストで固定できる） |
| 動きは CSS・`rAF` を使わない | 18席を毎フレーム描き直すと重い |
| `setInterval` は秒数の1か所だけ | 席の一覧側に置くと席の数だけ重くなる |
| `will-change`／`backdrop-filter` なし | 新項目19（合成レイヤーを増やさない） |
| `prefers-reduced-motion` で全部止める | 止めても席と状態の印は読める |
| 席は透明な面で覆う | 線だけだと線の上しか押せず、指では押せない |
| 動いていない人に作業内容を書かない | 止まっている人の「作業中の文」は嘘になる |
| 色を足しても、動くのは `moving:true` だけ | 「緑＝いま動いている」の対応が崩れると、色が飾りになる |
| 動きを止めても実行中の印は濃いまま | 止めた瞬間に「誰が動いているか」が消えてはいけない |
