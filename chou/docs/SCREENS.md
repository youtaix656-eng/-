...
| `src/lib/conflicts.js` | 低FODMAP と反対になるものを**元データから毎回導く**（アダムスキー式・発酵食品・**善玉菌の餌**の3層）。どちらが正しいかは決めない |
...
| `src/data/prebiotics.js` | 善玉菌の餌14件（種類3つ）・**出典どうしの食い違い3件**・出典の誤り4件の訂正・裏が取れていない主張7件・オメガ3・リンゴ酢 |
| `src/lib/tocCandidates.js` | 受け入れ前の4つの確かめ・見送り・取り消し（純粋な関数） |

### テストで機械チェックしていること（`chou/test/*.test.mjs`・151件）
