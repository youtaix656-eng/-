...
| `src/lib/digest.js` | 横断のまとめ。**元データから毎回集めるだけ**（訂正66件・裏が取れていない主張106件・食い違い12か所・扱わないこと3件・出典17本）。素材を足したら `DIGEST_SUBJECTS` にも足す |
| `src/components/Digest.jsx` | 「まとめて見る」画面。数えて並べるだけで、採点も順位付けもしない |
| `src/components/RedFlagLink.jsx` | 受診の目安への導線の**共通部品**。画面ごとに文言を書かない（決まり19・74） |
| `src/lib/gaps.js` | まだ書いていない日。**責めない・連続日数を数えない**（決まり77） |
| `src/lib/periods.js` | いつもと違う期間の印。**予測も判定もしない**（決まり79） |
| `src/lib/compare.js` | 並べるだけの層（食べた日／曜日／暮らし／期間／やめてみた前後／整腸剤）。**矢印を書かない**（決まり80） |
| `src/lib/weekly.js` | 今週と先週。**良くなった・悪くなったを返さない**（決まり81） |
| `src/lib/visits.js` | 通院の予定・聞きたいこと・受診のあと。**鳴らさない**（決まり82） |
| `src/lib/share.js` | 印刷・共有・書き出し。**共有が使えなければダウンロードへ落とす**（決まり83） |
| `src/lib/csv.js` | CSV。**数式にさせない・BOM・判定の列を作らない**（決まり84） |
| `src/lib/transfer.js`／`src/lib/qr.js` | 別の端末へ渡す。**入らない QR を入ったふりしない**（決まり85） |
| `src/lib/errorLog.js` | 端末内のエラー。**外へ送らない・人を責めない**（決まり86） |
| `src/lib/speak.js`／`src/lib/voice.js` | 読み上げ・音声入力。**どちらも既定オフ**（決まり88・89） |
| `src/data/diseases.js` | お腹の病気10件。**当てはめる仕掛けを作らない** |
| `src/data/breathing.js` | お腹で息をする5件・なでる4件・やめどき。**やめどきを先に置く** |
| `src/data/ibsCare.js` | 型ごとにできること4件。**型は自分で選ぶ**（記録から当てない） |
| `src/data/eatingOut.js` | 外食・中食8件。**お店と商品の名前を持たない・値段の話をしない** |
| `src/data/flora.js` | 腸内フローラの言葉6件・訂正3件・裏が取れていない主張4件 |
| `src/components/Finder.jsx`／`ErrorBoundary.jsx`／`ScrollArrows.jsx` | どの画面からも使う共通部品 |
| `public/manifest.webmanifest`／`public/sw.js` | 端末に入れる・オフラインで開く。**通知は持たない** |
| `src/data/knowMenu.js` | 「しらべる」の一覧（26件×4まとまり）。**画面に一覧を書かない**（決まり94） |

### テストで機械チェックしていること（`chou/test/*.test.mjs`・278件）
...
| まとめが元データから導かれる（手書きの一覧・件数の書き写しが無い） | 決まり68・69 |
| まとめで並べ替えない・「訂正が多い＝悪い」と読ませない | 決まり70 |
| 横断のまとまりが語の一致で拾っていない・空箱を出さない | 決まり71 |
| 検査を勧めない・水分に「1日◯リットル」を持たない・体重を記録しない | 決まり72 |
| 本文が元の項目の言い方のまま（`correction`／`withdrawn`／`note`） | 決まり73 |
| 受診の目安への導線が、全画面のファイルに入っている | 決まり19・74 |
| 見張りがコメント行・id の語で誤って落ちない | 決まり75 |
| 水分は段だけ・量の基準を持たない | 決まり76 |
| 空いている日の文に「さぼ・怠け・連続・途切」が出ない | 決まり77 |
| 期間の印が、予測も判定もしない・知らない種類を作らない | 決まり79 |
| 並べる層に矢印も割合の計算も無い・足りない時に黙らない | 決まり80 |
| 週次に平均も連続日数も無い・差の言い方が良し悪しを言わない | 決まり81 |
| 通院に通知の仕掛けが無い・言われたことを整えない | 決まり82 |
| 印刷が白地に黒字・貼った文字をタグとして読ませない | 決まり83 |
| CSV が数式にならない・BOM が付く・判定の列が無い | 決まり84 |
| 受け渡しが壊れていたら理由を返す・入らない QR を出さない | 決まり85 |
| エラーの記録が外へ送らない・20件で止まる・人を責めない | 決まり86・87 |
| 読み上げ・音声入力が、使えない端末で使えるふりをしない | 決まり88・89 |
| 読みやすさが根っこの属性だけで効く・印刷で白地に反転する | 決まり90 |
| PWA 一式がそろい、`sw.js` が通知を持たない | 決まり91 |
| 新しい素材が `DIGEST_SUBJECTS` にも入っている | 決まり93 |
| しらべるの一覧がデータから導かれる・まとまりは4つ | 決まり94 |
| 同じ画面への入口が2つ無い・読みを手で持つ・URLをデータに書かない | 決まり95・96・97 |
| App が開ける画面すべてに、ナビか しらべる からの道がある | 決まり94 |
| 記録の道具がカレンダーからも行ける・判定しないと添えてある | 決まり98 |
