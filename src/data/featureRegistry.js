...
const featureRegistry = [
  ...
  { id: 'keiketsulibrary', title: '経絡経穴 教科書ライブラリ', icon: '📚', category: '学習・演習', view: 'keiketsulibrary', desc: '経絡経穴の教科書ページを貼り付けて端末内に保存し、タイトル・本文で検索できる（教科書原文の置き場）。貼り付けた原文は自動で問題化されない（著作権のある文章をそのまま出題プールに常設しない方針のため）。実際の経穴データへ反映するにはこの原文を出典としてCLAUDEが手動整備する。自動生成の「経穴カード→経絡／取穴部位」テンプレート（generator.jsのkeiketsuMeridian・keiketsuLocation）もkeiketsuCards.jsの検証済みデータをそのまま使う。フラッシュカード画面（経穴カードモード）から遷移。', tags: ['経絡経穴', '教科書', 'ライブラリ', '検索'] },
